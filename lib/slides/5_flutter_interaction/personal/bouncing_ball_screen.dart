...
class _BouncingBallScreenState extends State<BouncingBallScreen>
  ...
  Size _getSize() {
    if (_widgetSizeKey.currentContext != null) {
...
class _SketchPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (block != null) {
      ...
      shadowPath.moveTo(block!.maxX, block!.maxY);
      shadowPath.lineTo(block!.maxX - xGap, block!.maxY + yGap);
      shadowPath.lineTo(block!.minX - xGap, block!.maxY + yGap);
      shadowPath.lineTo(block!.minX, block!.maxY);
      shadowPath.close();

      canvas.drawPath(shadowPath, _shadowPaint);

      final wallPath = Path();
      wallPath.moveTo(block!.minX, block!.minY);
      wallPath.lineTo(block!.minX, block!.maxY);
      wallPath.lineTo(block!.minX - xGap, block!.maxY + yGap);
      wallPath.lineTo(block!.minX - xGap, block!.maxY + yGap - block!.height);
      wallPath.close();

      canvas.drawPath(wallPath, _wallPaint);
    }
