...
class SprayDrawingScreen extends StatefulWidget {
  const SprayDrawingScreen({Key? key}) : super(key: key);

  @override
  State<SprayDrawingScreen> createState() => _SprayDrawingScreenState();
}

class _SprayDrawingScreenState extends State<SprayDrawingScreen> {
  ...
  final double _selectedWidth = 10;
...
class SprayPainter extends CustomPainter {
  ...
  void _edgeSmoothing(Canvas canvas, Size size) {
    ...
    for (final line in lines) {
      ...
      for (var i = 0; i < line.path.length; i++) {
        final currentPoint = line.path[i];
        final dist = distanceBetween(lastPoint, currentPoint);
        final angle = angleBetween(lastPoint, currentPoint);

        for (var i = 0; i < dist; i += 5) {
          final x = lastPoint.dx + (math.sin(angle) * i);
          final y = lastPoint.dy + (math.cos(angle) * i);
  ...
  void _bezier(Canvas canvas, Size size) {
    ...
    for (final line in lines) {
      ...
      for (var i = 0; i < line.path.length - 1; i++) {
        ...
        final midPoint = midPointBetween(p1, p2);
